...
const GeneralKnowledgeExpressTestCard = ({
    ...
    useEffect(() => {
        ...
        if (
            randowGeneralKnowledgeExpressTestData.correctAnswerIndex === selectedAnswer &&
    ...
    useEffect(() => {
        if (selectedAnswer === randowGeneralKnowledgeExpressTestData.correctAnswerIndex) {
            setCountCorrectAnswers((prevCount) => prevCount + 1);
        }
    }, [
        selectedAnswer,
        randowGeneralKnowledgeExpressTestData.correctAnswerIndex,
        setCountCorrectAnswers,
    ]);
    ...
    return (
        <div className="flex flex-col items-center">
            <div className="text-cyan-900 md:text-3xl mb-4 font-lato">
                {index + 1} / {generalKnowledgeExpressTestQuestionQty}
            </div>
            <div className="border border-teal-800 rounded-lg p-4 md:p-12 flex flex-col items-center">
                {randowGeneralKnowledgeExpressTestData.image && (
                    <img
                        className="w-24 h-24 md:w-48 md:h-48 object-contain"
                        src={randowGeneralKnowledgeExpressTestData.image}
                ...
                <div className="flex flex-shrink justify-center w-48 md:w-96 mt-4 text-cyan-900 text-sm md:text-xl font-playfair font-bold text-center">
                    {randowGeneralKnowledgeExpressTestData.question}
                </div>
                <div className="mt-8 text-cyan-900 text-sm md:text-lg font-playfair flex flex-col items-start">
                    {randowGeneralKnowledgeExpressTestData.answers.map(
                        (answer, idx) => (
                            <TestRadioButton
                                key={idx}
                ...
                {correctOrWrongAnswerMessage === "Wrong" && (
                    <div className="mt-4 flex flex-col items-center">
                        ...
                        <div className="mt-2 flex justify-center text-center px-4 py-2 bg-green-50 border border-green-300 rounded-lg text-green-800 text-sm md:text-xl font-playfair font-bold shadow-sm">
                            {
                                randowGeneralKnowledgeExpressTestData.answers[
                                    randowGeneralKnowledgeExpressTestData
                                        .correctAnswerIndex
                                ]
                            }
...
export default GeneralKnowledgeExpressTestCard;
